...
#define NGEN2 1000
#define NBIN 100
#define XMAX 90.0 //I changed XMAX to show appropriate graphs and histogram
#define XMIN 0.0
...
{
	return ( (TMath::Exp(TMath::Log(1)- n[0]*0.5*TMath::Log(2.0) - gammln(n[0]*0.5)
	+ (n[0]*0.5-1.0)*TMath::Log(x[0]) - x[0]*0.5)) - (par[0]*1/(TMath::Sqrt(2.0*3.1415926)*par[2])
	*TMath::Exp(-(x[0]-par[1])*(x[0]-par[1])/(2.0*(par[2])*(par[2])))) )
...
TF1 * fun3 = new TF1 ("fun3", fraction, XMIN, XMAX, 3);
fun3 -> SetParameters (1.0, 28.0, 6.7);
fun3 -> Draw("LSAME"); //black
