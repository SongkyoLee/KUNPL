...
Double_t interference( Double_t *x, Double_t *par)
{
   ...
   Double_t yn_p2 = (x[1]) * (x[1]);       // adding new interference, say yn.
   ...
   Double_t amplitude = par[0] * (cos(tpi_l  * sqrt(x_p2 + ym_p2))
		      ...
		      + cos(tpi_l * sqrt(x_p2 + yn_p2)));   // also correcting the eqns.
...
void Prob02( Double_t d = 3, Double_t lambda = 1, Double_t amp = 10)
{
   TCanvas *c1 = new TCanvas("waves", "A triple slit experiment", 300,40, 1004, 759);
   c1->Range(0, -10,  30, 10);
   c1->SetFillColor(0);
   ...
   TLatex title;
   title.DrawLatex(1.6, 8.5, "A triple slit experiment");
   ...
   line = new TLine(15, 0 - 0.5*d +0.2 ,15, 0-0.2);
   line->SetLineWidth(10); line->Draw();
   line = new TLine(15, 0+0.2 ,15, 0 + 0.5*d -0.2);
   line->SetLineWidth(10); line->Draw();
   line = new TLine(15,0 + 0.5*d + 0.2,15, 10);
   line->SetLineWidth(10); line->Draw();
   
   // I divided the original TLine(15, 0 - 0.5*d +0.2 ,15, 0 + 0.5*d -0.2) into two 
   // so that there is another slit in the middle y=0, with width 0.4 (0-0,2 ~ 0+0.2)
   ...
   for (Int_t i = 0; i < 16; i++) {
      ...
      arc->DrawArc(0, 0, r, 0., 360, "only");    // yellow halfcircle from added middle slit.
      arc->DrawArc(0, -0.5*d, r, 0., 360., "only");
      r += dr;
   }
