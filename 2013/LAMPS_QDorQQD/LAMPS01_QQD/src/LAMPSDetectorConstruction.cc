...
*/
  ...
  //===================================
	...
	const G4double FPD_DX = 2.*m;
	const G4double FPD_DY = 0.5*m;
	const G4double FPD_DZ = 0.2*m;
	const G4double FPD_ROT_ANGLE = -70*deg; //temp
//	const G4double d_FROM_CENTER_FPD = 5*m; //temp
  ...
  G4RotationMatrix* FPD_ROTATE = new G4RotationMatrix();
	...
	new G4PVPlacement(FPD_ROTATE, G4ThreeVector(1.9*m, 0, 5.2*m), "FPD_PV", FPD_LV, expHall, FALSE, 0);
//	new G4PVPlacement(FPD_ROTATE, G4ThreeVector(0, 0, d_FROM_CENTER_FPD), "FPD_PV", FPD_LV, expHall, FALSE, 0);
  ...
  const G4double Q1_DX =   2400*mm; //temporary
  const G4double Q1_DY =   2400*mm; 
  const G4double Q1_DZ =   500*mm;
	const G4double Q1_OPENING_R = 300*mm;
	const G4double Q1_OPENING_DZ = Q1_DZ;
	const G4double Q1_OPENING_DPHI = 360.*deg;
	const G4double d_FROM_CENTER_Q1 = 1750*mm;  

	const G4double Q2_DX =   2400*mm; //temporary
  const G4double Q2_DY =   2400*mm;
  const G4double Q2_DZ =   500*mm;
	const G4double Q2_OPENING_R = 400*mm;
	const G4double Q2_OPENING_DZ = Q2_DZ;
	const G4double Q2_OPENING_DPHI = 360.*deg;
	const G4double d_FROM_CENTER_Q2 = 2450*mm;  
 ...
 //KYO - temporary
	...
	const G4double DP_HALF_WIDTH = 1200*mm; //temp
	const G4double DP_R_CENT = 2200*mm;
	...
	const G4double DP_DEFLECTION_ANGLE = 55*deg;
...
//  G4LogicalVolume* DP_LV = new G4LogicalVolume(DP_tub, Universal_Vacuum, "DP_LV", 0, 0, 0);
  G4LogicalVolume* DP_LV = new G4LogicalVolume(DP_tub, Universal_Vacuum, "DP_LV", 0, 0, 0); // KYO - temp
  G4PVPlacement* DP_PV = new G4PVPlacement(DPP_ROTATE,
// 						 G4ThreeVector(-DP_R_CENT,0,2900*mm).rotateX(90*deg).rotateZ(180*deg),
 						 G4ThreeVector(DP_R_CENT,0,2900*mm),
						 "DP_PV",
						 DP_LV,
						 expHall,
						 false,
  ...
  const G4double DP_STRENGTH = -0.2942357687*tesla; //temp
