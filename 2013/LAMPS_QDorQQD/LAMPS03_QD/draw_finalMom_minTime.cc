...
void draw_finalMom_minTime()
{
	...
	//TH2D *hist_yz = new TH2D("hist_yz", "theta vs x on FPD;x(mm);theta(mrad)",23000, 5700, 8000, 1000, 0, 1000); //for FPD frame
	//TH1D *hist_z = new TH1D("hist_z", "z position on FPD;z(mm);Counts/binWidth",230,5700,8000);
	TH1D *hist_z = new TH1D("hist_z", "momentum on FPD;p(MeV/c);Counts/binWidth",700,160,230);
...
//	cout << "corresponding pointTime="<< pointTime<<", pointZ="<<pointZ <<", pointY"<< pointY << endl;
	...
	gStyle->SetOptStat(0);
	hist_z->Draw();
	c1->SaveAs("finalMom_minTime.pdf");
	//c1->Clear();
	//c2->cd();
	//hist_z->Draw();
