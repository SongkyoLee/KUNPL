...
void draw_xAng_xPos_minTime()
{
	...
	TH2D *hist_yz = new TH2D("hist_yz", "#theta_{f} vs x_{f} ;x_{f} (mm);#theta_{f} (mrad)",23000, 5700, 8000, 2500, 0, 250); //for FPD frame
...
//	cout << "corresponding pointTime="<< pointTime<<", pointZ="<<pointZ <<", pointY"<< pointY << endl;
	if(pointPx>=0 && pointPz>=0){
		pointTheta=( TMath::ATan(pointPz/pointPp)+(28*TMath::Pi()/180) )*1000; //rad to mrad
	} else if (pointPx>=0 && pointPz<0){
		pointTheta=( TMath::ATan(pointPz/pointPp)-(28*TMath::Pi()/180) )*1000; //rad to mrad
	} else { cout << " ********** Warning : momentum direction is weird!!! ********** " << endl; }
	hist_yz -> Fill(pointZ+pointX*(TMath::Tan(33.*TMath::Pi()/180)),pointTheta);
	//hist_z->Fill(pointZ+pointX*(TMath::Tan(33.*TMath::Pi()/180)),0.1);
*/
	...
	gStyle->SetOptStat(0);
	hist_yz->Draw();
	c1->SaveAs("xAng_xPos_minTime.pdf");
	//c1->Clear();
	//c2->cd();
	//hist_z->Draw();
