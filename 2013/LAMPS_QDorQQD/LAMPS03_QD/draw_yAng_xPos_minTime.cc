...
void draw_yAng_xPos_minTime()
{
	...
	TH2D *hist_yz = new TH2D("hist_yz", "#phi_{f} vs x_{f} ;x_{f} (mm);#phi_{f} (mrad)",23000, 5700, 8000, 5000, -250, 250); //for FPD frame
...
*/
			...
			hist_yz->Fill(pointZ+pointX*(TMath::Tan(33.*TMath::Pi()/180)),pointPhi);
//			hist_yz->Fill(pointZ+pointX*(TMath::Tan(33.*TMath::Pi()/180)),pointTheta);
	...
	gStyle->SetOptStat(0);
	hist_yz->Draw();
	c1->SaveAs("yAng_xPos_minTime.pdf");
	//c1->Clear();
	//c2->cd();
	//hist_z->Draw();
