...
void draw_xPos_zPos_minTime()
{
	...
	//TH2D *hist_xz = new TH2D("hist_xz", "x position vs z position on FPD;z(mm);x(mm)",35000, 7500,11000, 8000, -400, 400);
	TH2D *hist_xz = new TH2D("hist_xz", "x position vs z position on FPD;z(mm);x(mm)",20000, 4600,6600, 12000, 1800, 3000);
...
//			cout << "corresponding pointTime="<< pointTime<<", pointZ="<<pointZ <<", pointY"<< pointY << endl;
			hist_xz->Fill(pointZ,pointX);
			//hist_yz->Fill(pointZ+pointX*(TMath::Tan(56.78*TMath::Pi()/180)),pointY);
	...
	//	hist_yz->Fill(z+x*(TMath::Tan(56.78*TMath::Pi()/180)),y);
...
//	hist_yz -> Fill(pointZ+pointX*(TMath::Tan(56.78*TMath::Pi()/180)),pointY);
	...
	// draw the histogram
	c1->cd();
	//SimTree->Draw("y:z+x*(TMath::Tan(56.78*TMath::Pi()/180))","isFPD==1","");
	//SimTree->Draw("y:z+x*(TMath::Tan(56.78*TMath::Pi()/180))","isFPD==1&&pID==2212","");
	gStyle->SetOptStat(0);
	hist_xz->Draw();

	// save as PDF
	c1->SaveAs("xPos_zPos_minTime.pdf");
	//c1->Clear();
