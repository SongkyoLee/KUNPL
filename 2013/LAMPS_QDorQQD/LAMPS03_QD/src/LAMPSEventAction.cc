...
//  G4THitsCollection<DCHit> *CHC_DC = NULL;
  ...
  G4THitsCollection<FPDHit> *CHC_FPD = NULL;
...
//		if (useEventFile == 1)	anaRoot -> FillToF(particleNum, (G4int)pos_voxel.x(), parentID, pID, (G4int)pos_voxel.y(), EDep, ToF);
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="FPDHitCollection");
  if (HCTE)	CHC_FPD = (G4THitsCollection<FPDHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_FPD = NULL;

  if (CHC_FPD) {
	  G4int nHits = CHC_FPD -> entries();
	  
	  for (G4int i = 0; i != nHits; i++) {
			G4int parentID = (*CHC_FPD)[i] -> GetParentID();
			G4int pID = (*CHC_FPD)[i] -> GetParticleID();
			G4ThreeVector pos = (*CHC_FPD)[i] -> GetPosition();
			G4ThreeVector mom = (*CHC_FPD)[i] -> GetMomentum();
			G4double EDep = (*CHC_FPD)[i] -> GetEDep();
			G4double ToF = (*CHC_FPD)[i] -> GetToF();

//		std::ofstream outFile("all.out", std::ios::app);
//		outFile << pos.x() << " " << pos.y() << " " << pos.z() << " " << EDep << " " << ToF << G4endl;

//		if (useEventFile == 1)	anaRoot -> FillFPD(particleNum, (G4int)pos_voxel.x(), parentID, pID, (G4int)pos_voxel.y(), EDep, ToF);
			anaRoot -> FillFPD(particleNum, parentID, pID, pos.x(), pos.y(), pos.z(), mom.x(), mom.y(), mom.z(), EDep, ToF);
	  }
  }
