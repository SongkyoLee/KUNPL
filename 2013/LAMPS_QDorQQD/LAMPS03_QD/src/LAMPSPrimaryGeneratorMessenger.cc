...
:LAMPSPGA(LAMPSGun), runManager(runPointer)
{
	...
	useRandom -> SetGuidance("use [rndmKineticEnergy]&[rndm_p_direction]: 0(don't use) or 1(use:round) or 2(use:square)  (default:0)");
	...
	rndm_p_direction -> SetGuidance("For useRandom1, Set pYmin, pYmax, PHI : pXmin and pYmax are minimum and maximum polar angle of momentum direction respectively in degrees. It will be rotated PHI degrees along the z axis (default:14 24 360)");
	...
	rndm_p_direction_2 = new G4UIcmdWith3Vector("/lamps/rndm_p_direction_2", this);
	rndm_p_direction_2 -> SetGuidance("For useRandom2, Set pXcut, pYcut, pZcut : pX from -pXcut to + pXcut, pY from -pYcut to +pYcut (default:5 5 100)");
	rndm_p_direction_2 -> SetParameterName("pXcut", "pYcut", "pZcut", true);
	rndm_p_direction_2 -> SetDefaultValue(G4ThreeVector(5,5,100));
	rndm_p_direction_2 -> AvailableForStates (G4State_PreInit, G4State_Idle);
}
...
void LAMPSPrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
	...
	else if ( command == rndm_p_direction_2 )
	{
		LAMPSPGA -> SetRndm_p_direction( rndm_p_direction_2 -> GetNew3VectorValue(newValues) );
	}
