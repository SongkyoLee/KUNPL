...
//	G4cout << "=====GetFieldValue start!!=======" << G4endl;
	...
	const G4double Q1_F_GRADIENT = -(0.3766247388/200)*tesla; // unit = tesla/mm 
...
//			&&  Point[2]>=3.5*m 
			&&  Point[0] <= tan(65*Deg2Rad)*Point[2] - tan(65*Deg2Rad)*3.5*m 
			&& ( sqrt((Point[0]-1.8*m)*(Point[0]-1.8*m)+(Point[2]-3.5*m)*(Point[2]-3.5*m)) >= 0.5*m && sqrt((Point[0]-1.8*m)*(Point[0]-1.8*m)+(Point[2]-3.5*m)*(Point[2]-3.5*m)) <= 3.1*m )
//			&& Point[0] < -tan(35*Deg2Rad)*Point[2]+tan(35*Deg2Rad)*2.9*m+2.2*m )  {
			&& Point[0] <= -tan(10*Deg2Rad)*Point[2] + tan(10*Deg2Rad)*3.5*m + ( 1-cos(55*Deg2Rad)+tan(10*Deg2Rad)*sin(55*Deg2Rad) )*1.8*m  )  {
    Bfield[0]=0.*tesla;
//    Bfield[1]=F_STRENGTH;
...
//    Bfield[2] = 0.6*tesla; //KYO - solenoid 
	} else if( Point[2]<=(2050*mm+(500*mm/2)) && Point[2]>=(2050*mm-(500*mm/2)) && sqrt(Point[0]*Point[0] + Point[1]*Point[1]) <= 200*mm  ) {
