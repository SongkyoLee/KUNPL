...
*/
  ...
  //===================================
	...
	const G4double FPD_DX = 2.5*m;
	const G4double FPD_DY = 0.5*m;
	const G4double FPD_DZ = 0.2*m;
	const G4double FPD_ROT_ANGLE = -120*deg; //temp
//	const G4double d_FROM_CENTER_FPD = 5*m; //temp
  ...
  G4RotationMatrix* FPD_ROTATE = new G4RotationMatrix();
	...
	new G4PVPlacement(FPD_ROTATE, G4ThreeVector(1.5*m, 0, 5.4*m), "FPD_PV", FPD_LV, expHall, FALSE, 0);
//	new G4PVPlacement(FPD_ROTATE, G4ThreeVector(0, 0, d_FROM_CENTER_FPD), "FPD_PV", FPD_LV, expHall, FALSE, 0);
  ...
  const G4double Q1_DX =   2600*mm; //temporary
  const G4double Q1_DY =   2600*mm; 
  const G4double Q1_DZ =   500*mm;
	...
	const G4double d_FROM_CENTER_Q1 = 2050*mm;  
 ...
 //KYO - temporary
	...
	const G4double DP_HALF_WIDTH = 1300.*mm; //temp
  ...
  G4RotationMatrix* DPP_ROTATE = new G4RotationMatrix();
	...
	G4ThreeVector DP_EXIT_TRAN(-cos(DP_DEFLECTION_ANGLE)*DP_R_CENT + ((DP_SHIM_DY/2)/cos(10.*deg)),sin(DP_DEFLECTION_ANGLE)*DP_R_CENT,0);
