...
// Therefore, num of bin = (54-20)/bintWidth
	...
	const Int_t Narr = 27; //bidwidth=random (you should change this flexibly)
	...
	Double_t tL_CFD[Narr] = {0}; //lower boundary of bin -> larger value
	Double_t tU_CFD[Narr] = {0}; // upper boundary of bin -> smaller value
	...
	Double_t y_pre_CFD[Narr] = {0}; // y axis : counts (raw)
	Double_t y_CFD[Narr] = {0}; // y axis : counts (normalized)
	Double_t yE_CFD[Narr] = {0}; // y axis error
	Double_t xE_CFD[Narr] = {0}; // x axis error

	//diffrent bin width for different energy range (flexible)
	Double_t binWidthArr[] = {0.5, 1., 1.5, 2.0};
...
// loop for calculating KE of functional value (including upper and lower boundary)
	...
	{
		if (i<=10) binWidth=binWidthArr[1]; // E > 11 MeV regions
		else if (i>=11 && i <= 13) binWidth=binWidthArr[3];
		else if (i==14) binWidth=binWidthArr[1];
		else if (i==15 || i==16) binWidth=binWidthArr[2];
		else if (i==17) binWidth=binWidthArr[2];
		else if (i==18 || i==19 ) binWidth=binWidthArr[1];
		else if (i==20) binWidth=binWidthArr[3];
		else if (i==21 || i==22 || i==23) binWidth=binWidthArr[2];
		else  binWidth=binWidthArr[1];

		if (i==0) {
			t_CFD[i] = 20+ binWidth/2.; // neutron time distribution starts from t=20ns
			tU_CFD[i] = 20.;
			tL_CFD[i] = 20.+ binWidth;
		}
		else {
			t_CFD[i] = tL_CFD[i-1] + binWidth/2.;
			tU_CFD[i] =tL_CFD[i-1];
			tL_CFD[i] =tL_CFD[i-1] + binWidth;
		}
		...
		while(1)
		{
			...
			for(Int_t i=0; i<Narr; i++){
				if (energy >= EL_CFD[i] && energy < EU_CFD[i]) {
					...
					//cout << " Reading..." << endl;
					//cout << i<<"th E_CFD=" << E_CFD[i] <<", y_CFD= " << y_CFD[i] << endl;
				}else continue;
			}
		}
	}
...
//	fNEnergy_mean->Draw("samePE3");
	...
	legend->Draw();
	c->SaveAs("fNenergy.png");
	c->SaveAs("fNenergy.pdf");
