...
void correction()
{
	//variable to read in (from data_to_tree.root)
	Double_t lped;
	Double_t rped;
	Double_t acctl;
	Double_t acctr;
	...
	Double_t Al;
	Double_t Ar;
	// variable to write out (ti corrected_tree.root)
	Double_t TDCl;
	Double_t TDCr;
	Double_t ADCl;
	Double_t ADCr;
...
//ptfactor = point to time factor (change TDC channel to time(ns) by multiplying this value)
	...
	const Double_t ptfactor = 0.0407; //from Kisoo EXP
...
// Time Work Coreection function coefficient.
// When we use exponential fucntion, a is the slope and b is the constant : y = Exp[a*x]+b
/*
	//expo
...
*/
// When we use 1st polynomial fucntion, a is the slope and b is the constant : y = a*x+b
	//pol1
/*
	const Double_t al = 0.0020; //from Cobalt EXP Ben
	const Double_t ar = -0.0007;
	const Double_t bl = 10.9742;
	const Double_t br = 9.5995;
*/
	const Double_t al = 0.0123; //from Cobalt EXP Ben
	const Double_t ar = 0.0081;
	const Double_t bl = 9.7528;
	const Double_t br = 8.7788;

//	const Double_t gaptime = 4.67; //[ns] (when L=1.4m, c=0.3m/ns, t=L/c)
	const Double_t gaptime = 4.68157; //[ns] (when L=1.4035m, c=0.299792459m/ns, t=L/c)
//	const Double_t gaptime = 4.59; //[ns] (when L=1.375m, c=0.3m/ns, t=L/c)
...
//main process
	...
	//convert TDC to time and perform the TWC
...
//	pedrhist->Fit("gaus", "", "", 75, 175); //Ben
	...
	//Finally correct and write the accidental
...
//		accTDCr = acctr*ptfactor-ar*Ar-br-datart0mean+gaptime;
	...
	//Finally correct and write the data
