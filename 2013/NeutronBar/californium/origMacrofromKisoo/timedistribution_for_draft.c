...
void timedistribution_for_draft()
{
//define variables
...
//define file, tree...
	...
	californium->Project("hdata", "(TDCl+TDCr)/2", "TDCl<80&&TDCr<80");
	accidental->Project("hacc", "(accTDCl+accTDCr)/2", "accTDCl<80&&accTDCr<80");
...
//1.6875 is accidental rate. I achieved this value by equalize data taking time
		loop[i-LL] = datacount[i-LL]-TMath::Nint(1.39726*acccount[i-LL]);
...
//	c_after->SaveAs("VTD_totalfit_05a.png");
	c_after->SaveAs("VTD_totalfit_05a.pdf");
}
...
Double_t bg(Double_t* x, Double_t* par)
{
	if(x[0] >= 6 && x[0] < 25) return par[0]*TMath::Exp(par[1]*x[0]);
	else return 0.;
}
