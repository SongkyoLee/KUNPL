...
G4VPhysicalVolume* exampleDetectorROGeometry::Build()
{
 // G4Material* dummyMat = new G4Material("dummyMat", 1., 1*g/mole, 1*g/cm3);
  ...
  G4PVPlacement* ROLabPV = new G4PVPlacement(0, G4ThreeVector(), "ROLabPV",
						 ROLabLV, 0, false, 0);
  const G4int numOfXVoxels = 11; //SKChanged..
  const G4int numOfYVoxels = 11;
  const G4int numOfZVoxels = 1;
  ...
  G4VPhysicalVolume* RODetPV = new G4PVPlacement(0, detPosition, "RODetPV",
						RODetLV, ROLabPV, false, 0);

  //make voxel
  G4Box* RODetXVoxelSolid = new G4Box("RODetXVoxelSolid", (detX*layerNum)/numOfXVoxels/2,
							  detY/2,
							  detZ/2);//SKChanged..layerNum
  G4LogicalVolume* RODetXVoxelLV = new G4LogicalVolume(RODetXVoxelSolid,
							dummyMat,
							"RODetXVoxelLV");
  ...
  G4Box* RODetYVoxelSolid = new G4Box("RODetYVoxelSolid", (detX*layerNum)/numOfXVoxels/2,
							  detY/numOfYVoxels/2,
							  detZ/2);
  G4LogicalVolume* RODetYVoxelLV = new G4LogicalVolume(RODetYVoxelSolid,
							dummyMat,
							"RODetYVoxelLV");
  ...
  G4Box* RODetZVoxelSolid = new G4Box("RODetZVoxelSolid", (detX*layerNum)/numOfXVoxels/2,
							  detY/numOfYVoxels/2,
							  detZ/numOfZVoxels/2);
  G4LogicalVolume* RODetZVoxelLV = new G4LogicalVolume(RODetZVoxelSolid, dummyMat,
							"RODetZVoxelLV");
  new G4PVReplica ("RODetXVoxelPV",
		RODetZVoxelLV,
		RODetYVoxelPV,
		kZAxis,
		numOfZVoxels,
