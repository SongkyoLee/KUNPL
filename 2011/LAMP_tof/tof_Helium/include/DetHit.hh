...
class DetHit : public G4VHit
{
  private :
    ...
    G4double globalTime; //SKChanged..

  public :
    DetHit();
    DetHit(G4int, G4int, G4int, G4double, G4double); //SKChanged..
    virtual ~ DetHit();
    ...
    G4double GetGtime() const { return globalTime; };//SKChanged..
};
...
:G4VHit()
{
  ...
  globalTime = right.globalTime; //SKChanged..
}
