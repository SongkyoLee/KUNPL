...
G4VPhysicalVolume* exampleDetectorConstruction::Construct()
{
  ...
  G4LogicalVolume* labLV = new G4LogicalVolume(labSolid, HeliumGas, "labLV");
...
//  elHe = new G4Element("Helium", "He", 2, 4.002602*g/mole); //SK
  ...
  Vacuum = new G4Material("Galactic", universe_mean_density,3, kStateGas,
			 2.73*kelvin); //SKChanged..
    Vacuum -> AddElement(elN, 75.47/99.95);
    Vacuum -> AddElement(elO, 23.20/99.95);
    Vacuum -> AddElement(elAr, 1.28/99.95);

  HeliumGas = new G4Material("HeliumGas", 2, 4.002602*g/mole, 1.786e-4*g/cm3);
