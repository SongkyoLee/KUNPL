...
#include <fstream>
#include <iomanip>
#include <cmath>//SKChanged.. for calculating root
...
void exampleEventAction::EndOfEventAction(const G4Event* anEvent)
{
  ...
  if (HC_Det)
  {
    ...
    for (G4int i=0; i != numHits; i++)
    {
      ...
      G4double globalTime = (*HC_Det)[i] -> GetGtime();//SKChanged..


//SKChanged.. : travel length----------------------------------------------
      zLength = 8.0*m;
    ...
    outFile     //<< std::setw(5) << i+1
		...
		<< std::setw(15) << minLength/m
	        //<< std::setw(15) << energyDeposit
                << std::setw(15) << minTime/ns << G4endl; //SKChanged.. : show Hitnumber
  }
}
