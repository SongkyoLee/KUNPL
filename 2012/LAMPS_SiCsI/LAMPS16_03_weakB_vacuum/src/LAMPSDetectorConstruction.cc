...
// For cubic world volume
  ...
  const G4double DZ_SiT  = 400e-03*mm;	// Third
	const G4double DZ_SiV  = 300e-03*mm;  // veto (end of the scintillator)
 	const G4double DZ_CsI  = 130.0*mm;		//These DZ are along the rotated z axes.
	const G4double DZ_OFFSET_surface = 230.0*mm;	// from target to front surface of CsI
	const G4double DX1 = 10.0*mm; // x & y of front surface of CsI 
	const G4double DX2 = 15.0*mm; // x & y of back surface of CsI

  const G4double ANGLE = 19.0*degree; // polar angle of the center axis
  const G4int PADNUM = 8;              // # of pads

  // Voxel size (not half) - temporary
  const G4double X_VOXEL_SIZE_SiCsI = 10*mm;
  const G4double Y_VOXEL_SIZE_SiCsI = 10*mm;
  const G4double Z_VOXEL_SIZE_SiCsI = 10*mm;

  //===================================================
  // Silicon detector SiF - KYO
  //===================================================
    
  G4Box *SiF_Solid = new G4Box("SiF_Solid", DX1/2, DX1/2, DZ_SiF/2);
  G4LogicalVolume *SiF_LV = new G4LogicalVolume(SiF_Solid, Silicon, "SiF_LV");

	for(G4int j=0; j<PADNUM; j++)
	{
		for(G4int i=0; i<4; i++)
		{
			G4RotationMatrix* rot = new G4RotationMatrix;
			rot -> rotateZ(-(360/PADNUM)*j*degree);
			rot -> rotateX(ANGLE);
			rot -> rotateZ(-90*i*degree);

				new G4PVPlacement(rot, G4ThreeVector(DX1/2, DX1/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS-DZ_SiF/2)
													.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
													"SiF_PV", SiF_LV, expHall, false, (j+1)*100+i+1); 
				new G4PVPlacement(rot, G4ThreeVector(DX1/2, DX1*3/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS-DZ_SiF/2)
													.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
													"SiF_PV", SiF_LV, expHall, false, (j+1)*100+i+5); 
				new G4PVPlacement(rot, G4ThreeVector(DX1*3/2, DX1/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS-DZ_SiF/2)
													.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
													"SiF_PV", SiF_LV, expHall, false, (j+1)*100+i+9); 
				new G4PVPlacement(rot, G4ThreeVector(DX1*3/2, DX1*3/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS-DZ_SiF/2)
													.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
													"SiF_PV", SiF_LV, expHall, false, (j+1)*100+i+13); 
		}
	}
  ...
  //===================================================
  // Silicon detector SiS - KYO
  //===================================================
    
   G4Box *SiS_Solid = new G4Box("SiS_Solid", DX1/2, DX1/2, DZ_SiS/2);
	...
	for(G4int j=0; j<PADNUM; j++)
	{
		for(G4int i=0; i<4; i++)
		{
			...
			rot -> rotateZ(-90*i*degree);
				...
				new G4PVPlacement(rot, G4ThreeVector(DX1/2, DX1/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiS_PV", SiS_LV, expHall, false, (j+1)*100+i+1); 
				new G4PVPlacement(rot, G4ThreeVector(DX1/2, DX1*3/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiS_PV", SiS_LV, expHall, false,  (j+1)*100+i+5); 
				new G4PVPlacement(rot, G4ThreeVector(DX1*3/2, DX1/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiS_PV", SiS_LV, expHall, false,  (j+1)*100+i+9); 
				new G4PVPlacement(rot, G4ThreeVector(DX1*3/2, DX1*3/2, DZ_OFFSET_surface-DZ_SiT-DZ_SiS/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiS_PV", SiS_LV, expHall, false,  (j+1)*100+i+13); 
		}
	}
  G4VisAttributes* SiS_VisAtt = new G4VisAttributes(G4Colour(0., 1., 0.));
  SiS_LV -> SetVisAttributes(SiS_VisAtt);
    
  //===================================================
  // Silicon detector SiT - KYO
  //===================================================
    
  G4Box *SiT_Solid = new G4Box("SiT_Solid", DX1/2, DX1/2, DZ_SiT/2);
  G4LogicalVolume *SiT_LV = new G4LogicalVolume(SiT_Solid, Silicon, "SiT_LV");
	...
	for(G4int j=0; j<PADNUM; j++)
	{
		for(G4int i=0; i<4; i++)
		{
			...
			new G4PVPlacement(rot, G4ThreeVector(DX1/2, DX1/2, DZ_OFFSET_surface-DZ_SiT/2)
											.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
											"SiT_PV", SiT_LV, expHall, false, (j+1)*100+i+1); 
			new G4PVPlacement(rot, G4ThreeVector(DX1/2, DX1*3/2, DZ_OFFSET_surface-DZ_SiT/2)
											.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
											"SiT_PV", SiT_LV, expHall, false, (j+1)*100+i+5); 
			new G4PVPlacement(rot, G4ThreeVector(DX1*3/2, DX1/2, DZ_OFFSET_surface-DZ_SiT/2)
											.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
											"SiT_PV", SiT_LV, expHall, false, (j+1)*100+i+9); 
			new G4PVPlacement(rot, G4ThreeVector(DX1*3/2, DX1*3/2, DZ_OFFSET_surface-DZ_SiT/2)
											.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
											"SiT_PV", SiT_LV, expHall, false, (j+1)*100+i+13); 
		}
	}
  G4VisAttributes* SiT_VisAtt = new G4VisAttributes(G4Colour(0., 1., 0.));
  SiT_LV -> SetVisAttributes(SiT_VisAtt);
    
  //===================================================
  // CsI scintillator - KYO
  //===================================================

	// ~~~~~~~~~~ for (1) ~~~~~~~~~~~~~~~~~~
  G4Trap *CsI1_solid = new G4Trap("CsI1_solid", DZ_CsI/2,
														(TMath::ATan(TMath::Sqrt((DX2/2-DX1/2)*(DX2/2-DX1/2)+(DX2/2-DX1/2)*(DX2/2-DX1/2))/
														(DZ_CsI)))*(180/TMath::Pi())*degree, 
														TMath::ATan((DX2/2-DX1/2)/(DX2/2-DX1/2))*(180/TMath::Pi())*degree, 
														DX1/2, DX1/2, DX1/2, 0*degree, DX2/2, DX2/2, DX2/2, 0*degree);
  G4LogicalVolume *CsI1_LV = new G4LogicalVolume(CsI1_solid, CsI, "CsI1_LV");
	// ~~~~~~~~~~ for (5) ~~~~~~~~~~~~~~~~~~
  G4Trap *CsI2_solid = new G4Trap("CsI2_solid", DZ_CsI/2,
														(TMath::ATan(TMath::Sqrt((DX2/2-DX1/2)*(DX2/2-DX1/2)+(DX2/2)*(DX2/2))/(DZ_CsI)))
														*(180/TMath::Pi())*degree, 
														TMath::ATan((DX2/2)/(DX2/2-DX1/2))*(180/TMath::Pi())*degree, 
														DX1/2, DX1/2, DX1/2, 0*degree, DX2/2, DX2/2, DX2/2, 0*degree);
  G4LogicalVolume *CsI2_LV = new G4LogicalVolume(CsI2_solid, CsI, "CsI2_LV");
	// ~~~~~~~~~~ for (9) ~~~~~~~~~~~~~~~~~~
  G4Trap *CsI3_solid = new G4Trap("CsI3_solid", DZ_CsI/2,
														(TMath::ATan(TMath::Sqrt((DX2/2-DX1/2)*(DX2/2-DX1/2)+(DX2/2)*(DX2/2))/(DZ_CsI)))
														*(180/TMath::Pi())*degree,
														 TMath::ATan((DX2/2-DX1/2)/(DX2/2))*(180/TMath::Pi())*degree,
													 	DX1/2, DX1/2, DX1/2, 0*degree, DX2/2, DX2/2, DX2/2, 0*degree);
  G4LogicalVolume *CsI3_LV = new G4LogicalVolume(CsI3_solid, CsI, "CsI3_LV");
	// ~~~~~~~~~~ for (13) ~~~~~~~~~~~~~~~~~~
  G4Trap *CsI4_solid = new G4Trap("CsI4_solid", DZ_CsI/2,
														(TMath::ATan(TMath::Sqrt((DX2/2)*(DX2/2)+(DX2/2)*(DX2/2))/(DZ_CsI)))
														*(180/TMath::Pi())*degree, 
														TMath::ATan((DX2/2)/(DX2/2))*(180/TMath::Pi())*degree,
														DX1/2, DX1/2, DX1/2, 0*degree, DX2/2, DX2/2, DX2/2, 0*degree);
  G4LogicalVolume *CsI4_LV = new G4LogicalVolume(CsI4_solid, CsI, "CsI4_LV");
	...
	for(G4int j=0; j<PADNUM; j++)
	{
		for(G4int i=0; i<4; i++)
		{
			...
			new G4PVPlacement(rot, G4ThreeVector(DX1/2+(DX2-DX1)/4, DX1/2+(DX2-DX1)/4, DZ_OFFSET_surface+DZ_CsI/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"CsI_PV", CsI1_LV, expHall, false, (j+1)*100+i+1);
			new G4PVPlacement(rot, G4ThreeVector(DX1/2+(DX2-DX1)/4, DX2+DX2/4, DZ_OFFSET_surface+DZ_CsI/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"CsI_PV", CsI2_LV, expHall, false, (j+1)*100+i+5);
			new G4PVPlacement(rot, G4ThreeVector(DX2+DX2/4, DX1/2+(DX2-DX1)/4, DZ_OFFSET_surface+DZ_CsI/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"CsI_PV", CsI3_LV, expHall, false,(j+1)*100+i+9);
			new G4PVPlacement(rot, G4ThreeVector(DX2+DX2/4, DX2+DX2/4, DZ_OFFSET_surface+DZ_CsI/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"CsI_PV", CsI4_LV, expHall, false, (j+1)*100+i+13);
			}
	}
  ...
  CsI3_LV -> SetVisAttributes(CsI_VisAtt);
  CsI4_LV -> SetVisAttributes(CsI_VisAtt);
    
  //===================================================
  // Silicon detector SiV - KYO
  //===================================================
    
  G4Box *SiV_Solid = new G4Box("SiV_Solid", DX2/2, DX2/2, DZ_SiV/2);
	G4LogicalVolume *SiV_LV = new G4LogicalVolume(SiV_Solid, Silicon, "SiV_LV");
	...
	for(G4int j=0; j<PADNUM; j++)
	{
		for (G4int i=0; i<4; i++)
		{
			...
			new G4PVPlacement(rot, G4ThreeVector(DX2/2, DX2/2, DZ_OFFSET_surface+DZ_CsI+DZ_SiV/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiV_PV", SiV_LV, expHall, false, (j+1)*100+i+1); 
			new G4PVPlacement(rot, G4ThreeVector(DX2/2, DX2*3/2, DZ_OFFSET_surface+DZ_CsI+DZ_SiV/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiV_PV", SiV_LV, expHall, false,(j+1)*100+i+5); 
			new G4PVPlacement(rot, G4ThreeVector(DX2*3/2, DX2*3/2, DZ_OFFSET_surface+DZ_CsI+DZ_SiV/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiV_PV", SiV_LV, expHall, false, (j+1)*100+i+13); 
			new G4PVPlacement(rot, G4ThreeVector(DX2*3/2, DX2/2, DZ_OFFSET_surface+DZ_CsI+DZ_SiV/2)
												.rotateZ(90*i*degree).rotateX(-ANGLE).rotateZ((360/PADNUM)*j*degree), 
												"SiV_PV", SiV_LV, expHall, false, (j+1)*100+i+9); 
	 }
	}
  G4VisAttributes* SiV_VisAtt = new G4VisAttributes(G4Colour(0., 1., 0.));
  SiV_LV -> SetVisAttributes(SiV_VisAtt);
...
//  const G4double NEW_CENTER = 320.*cm;
  ...
  G4Trd *HB1_INSIDE_V_DPA1 = new G4Trd("Helium Bag INSIDE DPA1",
				       ...
				       HB1_INSIDE_Z_DPA1/2);
  G4LogicalVolume *HB1_INSIDE_LV_DPA1 = new G4LogicalVolume(HB1_INSIDE_V_DPA1, Universal_Vacuum, "HB1_INSIDE_LV_DPA1");
  new G4PVPlacement(0,
 		    G4ThreeVector(),
		    "HB1_INSIDE_PV_DPA1",
		    HB1_INSIDE_LV_DPA1,
  ...
  G4Trd *HB_INSIDE_V_DPA2 = new G4Trd("Helium Bag INSIDE DPA2",
			       ...
			       HB_INSIDE_Z_DPA2/2);
  G4LogicalVolume *HB_INSIDE_LV_DPA2 = new G4LogicalVolume(HB_INSIDE_V_DPA2, Universal_Vacuum, "HB_INSIDE_LV_DPA2");
  new G4PVPlacement(0,
 		    G4ThreeVector(),
		    "HB_INSIDE_PV_DPA2",
		    HB_INSIDE_LV_DPA2,
...
*/
  ...
  const G4double DP_STRENGTH = -0.4*tesla;
  ...
  // ===========================================
	...
	SiCsISD* SiF_SD = new SiCsISD(SDname="/SiCsI/SiF_SD", "SiF");
	SiF_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(SiF_SD);
	SiF_LV -> SetSensitiveDetector(SiF_SD);

	SiCsISD* SiS_SD = new SiCsISD(SDname="/SiCsI/SiS_SD", "SiS");
	SiS_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(SiS_SD);
	SiS_LV -> SetSensitiveDetector(SiS_SD);

	SiCsISD* SiT_SD = new SiCsISD(SDname="/SiCsI/SiT_SD", "SiT");
	SiT_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(SiT_SD);
	SiT_LV -> SetSensitiveDetector(SiT_SD);

	SiCsISD* CsI_SD = new SiCsISD(SDname="/SiCsI/CsI_SD", "CsI");
	CsI_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(CsI_SD);
	CsI1_LV -> SetSensitiveDetector(CsI_SD);
	CsI2_LV -> SetSensitiveDetector(CsI_SD);
	CsI3_LV -> SetSensitiveDetector(CsI_SD);
	CsI4_LV -> SetSensitiveDetector(CsI_SD);

	SiCsISD* SiV_SD = new SiCsISD(SDname="/SiCsI/SiV_SD", "SiV");
	SiV_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(SiV_SD);
	SiV_LV -> SetSensitiveDetector(SiV_SD);
