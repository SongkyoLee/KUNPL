...
#include "G4Colour.hh" //KYO LAMPS12
#include "TString.h" // KYO LAMPS12
#include "TMath.h" //KYO LAMPS12
...
#include "ToFSD.hh"
#include "LANDSD.hh"
#include "SiCsISD.hh" // KYO LAMPS12

#include <sstream>
using std::stringstream;

// For TPC ReadOut Geometry - Genie
#include "TPCROGeometry.hh" 
#include "DCROGeometry.hh" 
#include "SiCsIROGeometry.hh" // KYO LAMPS12
...
/////////////////////////////////////////////////////////
{
  ...
  // Silicon - KYO
  density = 2.33*g/cm3;
  G4Material* Silicon = new G4Material(name="Silicon", density, nel=1);
...
*/
  ...
  const G4double RIN_TPC_IN	= 450.*mm;
  ...
  const G4double ROUT_TPC_IF	= 450.2*mm;
  const G4double RIN_TPC_IF	= 450.*mm;
...
// For cubic world volume
 ...
 //===================================================
 // Si - CsI detector (cylindrical shape) - KYO LAMPS12
 //===================================================

 const G4double RIN_Si		= 0.*mm;
 const G4double ROUT_Si		= 400.*mm;
 const G4double RIN_CsI		= 0.*mm;
 const G4double ROUT_CsI	= 400.*mm;
 const G4double DPHI_SiCsI	= 360.*deg;
 const G4double DZ_Si		= 300e-3*mm;
 const G4double DZ_CsI		= 50*cm; //temporary	
 const G4double DZ_Si_OFFSET	= 30*cm; //temporary
 ...
 //===================================================
 // Silicon detector - KYO
 //===================================================

 G4VSolid* Si_Solid = new G4Tubs("Si_Solid", RIN_Si, ROUT_Si, DZ_Si/2, 0, DPHI_SiCsI);
 G4LogicalVolume* Si_LV = new G4LogicalVolume(Si_Solid, Silicon, "Si_LV");
 G4PVPlacement* Si_PV = new G4PVPlacement(0, G4ThreeVector(0, 0, DZ_Si_OFFSET), "Si_PV", Si_LV, expHall, FALSE, 0);

 G4VisAttributes* Si_VisAtt = new G4VisAttributes(true, G4Colour(0.,1.,0.));
 Si_LV -> SetVisAttributes(Si_VisAtt);

 //===================================================
 // CsI detector - KYO
 //===================================================

 G4VSolid* CsI_Solid = new G4Tubs("CsI_Solid", RIN_CsI, ROUT_CsI, DZ_CsI/2, 0, DPHI_SiCsI);
 G4LogicalVolume* CsI_LV = new G4LogicalVolume(CsI_Solid, CsI, "CsI_LV");
 G4PVPlacement* CsI_PV = new G4PVPlacement(0, G4ThreeVector(0, 0, DZ_Si_OFFSET+DZ_Si/2+DZ_CsI/2), "CsI_PV", CsI_LV, expHall, FALSE, 0);

 G4VisAttributes* CsI_VisAtt = new G4VisAttributes(true, G4Colour(0.,0.,1.));
...
*/
  ...
  SiCsISD* Si_SD = new SiCsISD(SDname="/SiCsI/Si_SD", "Si");
  Si_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
  SDman -> AddNewDetector(Si_SD);
  Si_LV -> SetSensitiveDetector(Si_SD);

  SiCsISD* CsI_SD = new SiCsISD(SDname="/SiCsI/CsI_SD", "CsI");
  CsI_SD -> SetROgeometry(SICSI_RO_GEOMETRY);
  SDman -> AddNewDetector(CsI_SD);
  CsI_LV -> SetSensitiveDetector(CsI_SD);
