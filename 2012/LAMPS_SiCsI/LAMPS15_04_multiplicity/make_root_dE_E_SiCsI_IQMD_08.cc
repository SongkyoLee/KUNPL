...
void make_root_dE_E_SiCsI_IQMD_08()
{
	TFile *openFile = new TFile("rawData_IQMD_08.root", "READ");

	Int_t eventID, trackID, pID, parentID, sEventID, sTrackID, isSiCsI, detNum, copyNum;
	Double_t hitTime, energyInit, energyDeposit;

	TTree *GeneratorTree08 = (TTree *) openFile -> Get("GeneratorData");

	TTree *SimulationTree08 = (TTree *) openFile -> Get("SimulationData"); //meaningless?
		SimulationTree08 -> SetBranchAddress("eventID", &sEventID);
		SimulationTree08 -> SetBranchAddress("trackID", &sTrackID);
		SimulationTree08 -> SetBranchAddress("parentID", &parentID);
		SimulationTree08 -> SetBranchAddress("isSiCsI", &isSiCsI);
		SimulationTree08 -> SetBranchAddress("hitTime", &hitTime);
		SimulationTree08 -> SetBranchAddress("energyDeposit", &energyDeposit);
		SimulationTree08 -> SetBranchAddress("detNum", &detNum);
		SimulationTree08 -> SetBranchAddress("copyNum", &copyNum);


	TFile *writeFile = new TFile("dE_E_SiCsI_IQMD_08.root", "RECREATE");
	...
	for (Int_t i = 0; i < numSplits; i++) {
		tempTree[i] = SimulationTree08 -> CopyTree(Form("eventID>=%d&&eventID<%d", 7000+i*numEvents/numSplits, 7000+(i+1)*numEvents/numSplits));
	...
	for (Int_t i = 7000; i < 7000+numEvents; i++) {
		TTree *tempTree1 = GeneratorTree08 -> CopyTree(Form("eventID==%d", i));
		...
		TTree *tempTree2 = SimulationTree08 -> CopyTree(Form("eventID==%d&&isSiCsI==1", i));
