...
void make_root_dE_E_SiCsI_IQMD_05()
{
	TFile *openFile = new TFile("rawData_IQMD_05.root", "READ");

	Int_t eventID, trackID, pID, parentID, sEventID, sTrackID, isSiCsI, detNum, copyNum;
	Double_t hitTime, energyInit, energyDeposit;

	TTree *GeneratorTree05 = (TTree *) openFile -> Get("GeneratorData");

	TTree *SimulationTree05 = (TTree *) openFile -> Get("SimulationData"); //meaningless?
		SimulationTree05 -> SetBranchAddress("eventID", &sEventID);
		SimulationTree05 -> SetBranchAddress("trackID", &sTrackID);
		SimulationTree05 -> SetBranchAddress("parentID", &parentID);
		SimulationTree05 -> SetBranchAddress("isSiCsI", &isSiCsI);
		SimulationTree05 -> SetBranchAddress("hitTime", &hitTime);
		SimulationTree05 -> SetBranchAddress("energyDeposit", &energyDeposit);
		SimulationTree05 -> SetBranchAddress("detNum", &detNum);
		SimulationTree05 -> SetBranchAddress("copyNum", &copyNum);


	TFile *writeFile = new TFile("dE_E_SiCsI_IQMD_05.root", "RECREATE");
	...
	for (Int_t i = 0; i < numSplits; i++) {
		tempTree[i] = SimulationTree05 -> CopyTree(Form("eventID>=%d&&eventID<%d", 4000+i*numEvents/numSplits, 4000+(i+1)*numEvents/numSplits));
	...
	for (Int_t i = 4000; i < 4000+numEvents; i++) {
		TTree *tempTree1 = GeneratorTree05 -> CopyTree(Form("eventID==%d", i));
		...
		TTree *tempTree2 = SimulationTree05 -> CopyTree(Form("eventID==%d&&isSiCsI==1", i));
