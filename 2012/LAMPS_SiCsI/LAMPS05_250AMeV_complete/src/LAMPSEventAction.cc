...
#include "ToFHit.hh"
#include "LANDHit.hh"
#include "SiCsIHit.hh" //KYO
#include "SiCsISD.hh" //KYO
...
//  }
  ...
  G4THitsCollection<SiCsIHit> *CHC_SiCsI = NULL; //KYO
...
// KYO - SiCsIHitCollection
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_1"); //Si1
...
// KYO			G4int detID = (*CHC_SiCsI)[i] -> GetDetID();
// KYO			G4ThreeVector pos = (*CHC_SiCsI)[i] -> GetPosition();
			G4ThreeVector prePos = (*CHC_SiCsI)[i] -> GetPrePosition(); //KYO
			G4ThreeVector postPos = (*CHC_SiCsI)[i] -> GetPostPosition(); //KYO
			G4ThreeVector pos_voxel = (*CHC_SiCsI)[i] -> GetVoxelPosition();
			G4double EDep = (*CHC_SiCsI)[i] -> GetEDep();
			G4double ToF = (*CHC_SiCsI)[i] -> GetToF();
			G4double detNum = (*CHC_SiCsI)[i] -> GetdetNum(); //KYO

//		std::ofstream outFile("SiCsI.out", std::ios::app); //KYO
//		outFile << std::setw(10) << parentID
//			<< std::setw(10) << pID
//			<< std::setw(10) << prePos.x()
//			<< std::setw(10) << prePos.y()
//			<< std::setw(10) << prePos.z()
//			<< std::setw(10) << EDep
//			<< std::setw(10) << ToF
//			<< std::setw(10) << detNum << G4endl;

//		if (useEventFile == 1)	anaRoot -> FillDC(particleNum, "A1", detID, parentID, pID, pos.x(), pos.y(), pos.z(), EDep, ToF);
			anaRoot -> FillSiCsI(particleNum, parentID, pID, prePos.x(), prePos.y(), prePos.z(), postPos.x(), postPos.y(), postPos.z(), EDep, ToF, detNum); //KYO 1,2,3,4??? temporary...
	  }
  }


  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_2");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
  ...
  if (CHC_SiCsI) {
	  ...
	  for (G4int i = 0; i != nHits; i++) {
			G4int parentID = (*CHC_SiCsI)[i] -> GetParentID();
			G4int pID = (*CHC_SiCsI)[i] -> GetParticleID();
//			G4int detID = (*CHC_SiCsI)[i] -> GetDetID();
//			G4ThreeVector pos = (*CHC_SiCsI)[i] -> GetPosition();
			G4ThreeVector prePos = (*CHC_SiCsI)[i] -> GetPrePosition();
			G4ThreeVector postPos = (*CHC_SiCsI)[i] -> GetPostPosition();
			G4ThreeVector pos_voxel = (*CHC_SiCsI)[i] -> GetVoxelPosition();
			G4double EDep = (*CHC_SiCsI)[i] -> GetEDep();
			G4double ToF = (*CHC_SiCsI)[i] -> GetToF();
			G4double detNum = (*CHC_SiCsI)[i] -> GetdetNum(); //KYO
...
//		if (useEventFile == 1)	anaRoot -> FillDC(particleNum, "A1", detID, parentID, pID, pos.x(), pos.y(), pos.z(), EDep, ToF);
			anaRoot -> FillSiCsI(particleNum, parentID, pID, prePos.x(), prePos.y(), prePos.z(), postPos.x(), postPos.y(), postPos.z(), EDep, ToF, detNum); //KYO
	  }
  }



  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_3");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
...
//		if (useEventFile == 1)	anaRoot -> FillDC(particleNum, "A1", detID, parentID, pID, pos.x(), pos.y(), pos.z(), EDep, ToF);
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_4");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
