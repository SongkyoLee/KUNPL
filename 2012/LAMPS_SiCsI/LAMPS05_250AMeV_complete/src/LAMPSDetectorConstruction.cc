...
#include "G4Colour.hh" //KYO
#include "TString.h" //KYO
#include "TMath.h" //KYO
...
#include "ToFSD.hh"
#include "LANDSD.hh"
#include "SiCsISD.hh" //KYO : for Si&CsI detector
...
// For cubic world volume
  ...
  //=============================================================
  // Si + CsI detector (inner side of TPC) - KYO (from Suhyun)
  //=============================================================

//    TString a; //??
    const G4double detS  = 300e-03*mm;	// length of a silicon pad (along rotated z direction)
    const G4double detC  = 100*mm;	// length of a scintillator (along rotated z direction)
    
    const G4double distS = 230*mm;	// distance from the target to the front surface (center) 
					// of 1st layer of CsI 
    ...
    const G4int pad = 8;              // # of pads (a layer)

  //===================================================
  // Silicon detector 1 (1st layer) - KYO
  //===================================================
    
    G4Box *detSolidS1 = new G4Box("detSolidS1", 
				distS*(1/TMath::Cos(coverage/2))
					*TMath::Sin(angle1-(coverage/2))
					*TMath::Tan((0.5*360/pad)*degree)*mm,
	 			distS*TMath::Tan(coverage/2)*mm,
				detS/2);

    G4LogicalVolume *detLVS1 = new G4LogicalVolume(detSolidS1, Silicon, "detLVS1");

    for(G4int i=0; i<pad; i++)
    { 
        new G4PVPlacement(new G4RotationMatrix(-(360/pad)*i*degree, -angle1, 0*degree),
			 G4ThreeVector(0*mm, 0*mm, (distS-(detS/2))*mm)
				.rotateX(-angle1).rotateZ(-(360/pad)*i*degree),
			 Form("detPVS1%d", i),
			 detLVS1,
			 expHall, //KYO
			 false,
   ...
   G4VisAttributes* Si_1_VisAtt = new G4VisAttributes(G4Colour(0., 1., 0.));
   detLVS1 -> SetVisAttributes(Si_1_VisAtt);
 
  //===================================================
  // Silicon detector 2 (2nd layer) - KYO
  //===================================================
    
    G4Box *detSolidS2 = new G4Box("detSolidS2",
				(distS+detC)*(1/TMath::Cos(coverage/2))
					*TMath::Sin(angle2-(coverage/2))
					*TMath::Tan((0.5*360/pad)*degree)*mm,
				(distS+detC)*TMath::Tan(coverage/2)*mm,
				detS/2);
    ...
    { 
        new G4PVPlacement(new G4RotationMatrix(-(360/pad)*i*degree, -angle2, 0*degree),
			G4ThreeVector(0*mm, 0*mm, (distS+detC-(detS/2))*mm)
				.rotateX(-angle2).rotateZ(-(360/pad)*i*degree),
			Form("detPVS2%d", i), 
			detLVS2,
			expHall, 
			false, 
   ...
   G4VisAttributes* Si_2_VisAtt = new G4VisAttributes(G4Colour(0., 1., 0.));
   detLVS2 -> SetVisAttributes(Si_2_VisAtt);
    
  //===================================================
  // CsI detector 1 (1st layer) - KYO
  //===================================================
    
    G4Trap *detSolidC1 = new G4Trap("detSolidC1",
				(detC/2)*mm, 
				0*degree, 
				0*degree,
        			distS*TMath::Tan(coverage/2)*mm,
    ...
    { 
        new G4PVPlacement(new G4RotationMatrix(-(360/pad)*i*degree, -angle1, 0*degree),
       			G4ThreeVector(0*mm, 0*mm, (distS+(detC/2))*mm)
				.rotateX(-angle1).rotateZ(-(360/pad)*i*degree),
			Form("detPVC1%d", i), 
			detLVC1, 
			expHall, 
   ...
   G4VisAttributes* CsI_1_VisAtt = new G4VisAttributes(G4Colour(0., 0., 1.));
   detLVC1 -> SetVisAttributes(CsI_1_VisAtt);
    
  //===================================================
  // CsI detector 2 (2st layer) - KYO
  //===================================================

    G4Trap *detSolidC2 = new G4Trap("detSolidC2",
				(detC/2)*mm, 
				0*degree, 
				0*degree,            
       				(distS+detC)*TMath::Tan(coverage/2)*mm,
					...
					*TMath::Tan((0.5*360/pad)*degree)*mm, 
				0*degree,
        			(distS+2*detC)*TMath::Tan(coverage/2)*mm,
        			(distS+2*detC)*(1/TMath::Cos(coverage/2))
					*TMath::Sin(angle2-coverage/2)
					*TMath::Tan((0.5*360/pad)*degree)*mm,
       				(distS+2*detC)*(1/TMath::Cos(coverage/2))
					*TMath::Sin(angle2-coverage/2)
    ...
    { 
        new G4PVPlacement(new G4RotationMatrix(-(360/pad)*i*degree, -angle2, 0*degree),
       			G4ThreeVector(0*mm, 0*mm,(distS+(3*detC/2))*mm)
				.rotateX(-angle2).rotateZ(-(360/pad)*i*degree),
			Form("detPVC2%d", i), 
			detLVC2, 
			expHall, 
   ...
   G4VisAttributes* CsI_2_VisAtt = new G4VisAttributes(G4Colour(0., 0., 1.));
   detLVC2 -> SetVisAttributes(CsI_2_VisAtt);
...
*/
  ...
  // ===========================================
	...
	SiCsISD* Si_SD_1 = new SiCsISD(SDname="/SiCsI/Si_SD_1", "1"); //KYO temporary...
	Si_SD_1 -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(Si_SD_1);
	detLVS1 -> SetSensitiveDetector(Si_SD_1);

	SiCsISD* CsI_SD_1 = new SiCsISD(SDname="/SiCsI/CsI_SD_1", "2");
	CsI_SD_1 -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(CsI_SD_1);
	detLVC1 -> SetSensitiveDetector(CsI_SD_1);

	SiCsISD* Si_SD_2 = new SiCsISD(SDname="/SiCsI/Si_SD_2", "3");
	Si_SD_2 -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(Si_SD_2);
	detLVS2 -> SetSensitiveDetector(Si_SD_2);

	SiCsISD* CsI_SD_2 = new SiCsISD(SDname="/SiCsI/CsI_SD_2", "4");
	CsI_SD_2 -> SetROgeometry(SICSI_RO_GEOMETRY);
	SDman -> AddNewDetector(CsI_SD_2);
	detLVC2 -> SetSensitiveDetector(CsI_SD_2);
