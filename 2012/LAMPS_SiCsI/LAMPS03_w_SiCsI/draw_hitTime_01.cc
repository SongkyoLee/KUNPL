...
void draw_hitTime_01()
{
	...
	TTree * tempTree[splitNum]
	for (Int_t i=0; i<splitNum; i++)
	{
		tempTree[i] = SimulationTree->CopyTree(Form("eventID>=%d&&eventID<%d",i*eventNum/splitNum, (i+1)*eventNum/splitNum));
	...
	for(Int_t i = 0; i < eventNum; i++)
	{
		...
		TTree* t_per_event = tree -> CopyTree(Form("eventID==%d&&isSiCsI==1&&pID==211&&parentID==0",i));

		for (Int_t j =0; j<entries; j++)
		{
			t_per_event -> GetEntry(j);

			if(isSiCsI==1&&pID==211&&parentIS==0)
			{
				h1 -> Fill(hitTime);
			}
		}
