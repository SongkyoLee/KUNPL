...
//		if (useEventFile == 1)	anaRoot -> FillDC(particleNum, "A2", detID, parentID, pID, pos.x(), pos.y(), pos.z(), EDep, ToF);
  ...
  if (CHC_ToF) {
	  ...
	  for (G4int i = 0; i != nHits; i++) {
			...
			G4ThreeVector mom = (*CHC_ToF)[i] -> GetMomentum(); //KYO
...
//		if (useEventFile == 1)	anaRoot -> FillToF(particleNum, (G4int)pos_voxel.x(), parentID, pID, (G4int)pos_voxel.y(), EDep, ToF);
			anaRoot -> FillToF(particleNum, (G4int)pos_voxel.x(), parentID, pID, (G4int)pos_voxel.y(), EDep, ToF, (G4double)mom.x(), (G4double)mom.y(), (G4double)mom.z());
	  }
  }
  ...
  if (CHC_ToF) {
	  ...
	  for (G4int i = 0; i != nHits; i++) {
			...
			G4ThreeVector pos = (*CHC_ToF)[i] -> GetPosition();
			G4ThreeVector pos_voxel = (*CHC_ToF)[i] -> GetVoxelPosition();
			G4ThreeVector mom = (*CHC_ToF)[i] -> GetMomentum();
			G4double EDep = (*CHC_ToF)[i] -> GetEDep();
			G4double ToF = (*CHC_ToF)[i] -> GetToF();
...
// KYO - SiCsIHitCollection
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_SiF");
...
//	G4cout << "SiCsI nHits = " << nHits << G4endl; 
	  ...
	  for (G4int i = 0; i != nHits; i++) {
			...
			G4int copyNum = (*CHC_SiCsI)[i] -> GetCopyNum();
			G4int detNum = (*CHC_SiCsI)[i] -> GetdetNum();

			anaRoot -> FillSiCsI(particleNum, parentID, pID, pos.x(), pos.y(), pos.z(), EDep, ToF, copyNum, detNum);
	  }
  }


  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_SiS");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
...
//	G4cout << "SiCsI nHits = " << nHits << G4endl; 
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_SiT");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
...
//	G4cout << "SiCsI nHits = " << nHits << G4endl; 
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_CsI");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
...
//	G4cout << "SiCsI nHits = " << nHits << G4endl; 
  ...
  hitCollectionID = SDManager -> GetCollectionID(HCname="SiCsIHitCollection_SiV");
  if (HCTE)	CHC_SiCsI = (G4THitsCollection<SiCsIHit> *)(HCTE -> GetHC(hitCollectionID));
  else		CHC_SiCsI = NULL;
